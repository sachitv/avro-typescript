...
describe("RecordType", () => {
  ...
  it("throws when missing required field during write for non-nested records", () => {
    ...
    assertThrows(
      () =>
        type.write(tap, { name: "Ann" } as unknown as Record<string, unknown>),
      Error,
      ...
      `Invalid value: 'undefined' for type: 
{
 ...
 at path: id`,
  ...
  it("maps writer field aliases to reader field names", () => {
    ...
    const reader = createRecord({
      name: "example.Person",
      fields: [{ name: "name", type: new StringType(), aliases: ["fullName"] }],
    });
...
`,
  ...
  it("throws when getting default for field without default", () => {
    ...
    assert(field);
    assertThrows(() => field.getDefault(), Error, "Field 'id' has no default");
  });
...
' for type: ${expectedTypeJson}`,
  ...
  it("throws on non-record values in toBuffer", () => {
    ...
    assertThrows(() => type.toBuffer(42 as unknown as Record<string, unknown>));
    assertThrows(() =>
      ...
      `Invalid value: 'undefined' for type: 
{
 ...
 at path: id`,
  ...
  it("throws when missing required field during compare", () => {
    ...
    assertThrows(() =>
      type.compare({ name: "Ann" } as unknown as Record<string, unknown>, valid)
    );
  });
});
