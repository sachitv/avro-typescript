...
export class SyncWritableTap extends TapBase implements SyncWritableTapLike {
   ...
   *
   * Performance optimization: Uses BigInt.asUintN(64, ...) to signal to V8's Turbofan
   * that all operations are on 64-bit values, enabling int64 lowering optimizations.
   * See https://groups.google.com/g/v8-reviews/c/SF2tmxAUpB8 and
   * https://v8.dev/blog/bigint#optimization-considerations for details.
   *
   ...
   * explicit 64-bit truncation.
   *
   * Note: Input values must be within int64 range (-2^63 to 2^63-1) per Avro spec.
